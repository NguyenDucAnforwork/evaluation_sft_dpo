...
from validator.evaluation.utils import create_dataset_entry
...
def load_results_dict(synthetic: bool = False):
    """Load existing evaluation results or create an empty dict if not found."""
    results_dict = {}
    results_path = cst.CONTAINER_EVAL_RESULTS_PATH_SYNTHETIC if synthetic else cst.CONTAINER_EVAL_RESULTS_PATH
    ...
    if os.path.exists(results_path):
        try:
            ...
            logger.error(f"Could not read existing results from {results_path}, starting fresh: {e}")

    return results_dict


def save_results_dict(results_dict, model_id=None, synthetic: bool = False):
